...
class allottask{
	...
	{   
    	...
    	/*foreach($task->fetchAll() as $t)
    	{
    		...
    		{   
    			...
    			echo "<label><a href='delete.php?delete=true&username=".$t['username']."' class=".$t['username']."><i class='glyphicon glyphicon-log-out'></i>Remove from Task</a></label>";
	...
	public function changetask($name,$work)
	{
		$stmt = $this->db->prepare("UPDATE taskalloted  SET work = :work WHERE username= :name");
    	//$work="NO Task";
    	//$given_by="";
    	$stmt->bindValue(":work", $work);
    	//$stmt->bindValue(":given_by", $given_by);
